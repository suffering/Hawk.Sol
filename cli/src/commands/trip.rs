...
use crate::ix::build_trip_ix;
use crate::rpc::{
    authority::check_trip_authority, execute_transaction, fetch_breaker_config, keypair::load_keypair,
};

pub fn run(global: &GlobalOpts, vault: Pubkey, yes: bool) -> Result<()> {
    if !confirm_action(&format!("Trip breaker for vault {vault}?"), yes)? {
        eprintln!("cancelled");
        return Ok(());
    }
    ...
    check_trip_authority(&signer.pubkey(), &config)?;

    let ix = build_trip_ix(&vault, &signer.pubkey());
